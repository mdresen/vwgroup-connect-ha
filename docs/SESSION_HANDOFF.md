...
| **Vehicle Data Scout** (Live API Monitor) | v1.8.13 | User-facing "Vehicle Data Scout" sensor that logs JSON fields the parser doesn't read, with masked sample values. Mirrors `tillsteinbach/CarConnectivity-*` "Unexpected Keys found" pattern that has been our richest source of API findings (CC-seatcupra #109, CC-skoda #50). Internally called "unexpected keys detector"; user-facing name is brand-localised (DE: "API-Beobachter", FR: "Détecteur de nouvelles données", etc. — see strings.json convention). Scope: `cariad/_unexpected_keys.py` with `EXPECTED_KEYS` dict per brand+endpoint; integration hook in each brand-client's `get_status`; structured WARN log with `VAG-UNEXPECTED:` prefix; diagnostics export field `unexpected_keys_log` (last N findings). Privacy: every sample value passes through `mask_vin`-style anonymisation. **Reporter Pipeline (1-click workflow):** when new fields detected → HA repair notification → user clicks "More info" → modal shows anonymised findings → user clicks "Report on GitHub" → browser opens pre-filled GitHub issue → user clicks Submit. ~30 seconds end-to-end. NO auto-push to external servers — opt-in telemetry is a separate v1.9.x scope (GDPR + HACS-Default-compliance + GitHub ToS). Enables crowd-sourced API discovery from diverse fleets (e.g. VW Golf 7 GTE, Audi S6 2021, Audi A4 2017). | new |
