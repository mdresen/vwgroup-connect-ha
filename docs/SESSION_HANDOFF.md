...
| **Unexpected Keys Detector** | v1.8.13 | Detection layer that logs JSON fields the parser doesn't read, with masked sample values. Mirrors `tillsteinbach/CarConnectivity-*` "Unexpected Keys found" pattern that has been our richest source of API findings (CC-seatcupra #109, CC-skoda #50). Scope: `cariad/_unexpected_keys.py` with `EXPECTED_KEYS` dict per brand+endpoint; integration hook in each brand-client's `get_status`; structured WARN log with `VAG-UNEXPECTED:` prefix; diagnostics export field `unexpected_keys_log` (last N findings). Privacy: every sample value passes through `mask_vin`-style anonymisation. Enables crowd-sourced API discovery — users with diverse fleets (e.g. VW Golf 7 GTE, Audi S6 2021, Audi A4 2017) can post anonymised reports as GitHub issues. | new |
