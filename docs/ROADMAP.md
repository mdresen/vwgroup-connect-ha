...
**Last updated:** 2026-05-03 — post v1.17.4 (Bruno-CI Stufe 2
**COMPLETE**: 80/80 strict-mode coverage all 3 brands — seat_cupra
35/35, skoda 24/24, cariad_bff 21/21. Drift-script erweitert mit
`{path_suffix}` placeholder + `_is_skipped_template()` filter für
v2 auto-fallbacks. CI-Workflow auf full `--strict` für alle Brands
ohne Whitelist). Plus Scope-Klarstellung: **Vehicle Data Scout
(v1.9.0) deckt 80% der Discovery-Cases** (neue Felder in known
endpoints) — mitmproxy/Frida-Capture-Stack reduziert auf P3
Edge-Case-Tool für komplett neue Endpoints / neue Brands /
neue App-Features. Nächste P0: v1.18.0 Push Bundle (FCM für
CUPRA/SEAT + MQTT für Skoda) ODER eigenes Lovelace-Card Repo
ODER weitere Bruno Quick-Wins.
