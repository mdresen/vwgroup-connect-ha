...
**Last updated:** 2026-05-02 — post v1.16.1 (PATCH: SEAT/CUPRA Climate
404 fix nach Gerhard's #53 + #122 SEAT scout-paths von r1150gs). #53
Phase 3 Phantom-Button bleibt offen — wartet auf Diagnostics für Root-
Cause. Nächste P0: v1.17.0 Skoda Vehicle-Info Bundle (vehicle-info +
renders + equipment + widget endpoints) + Cross-Brand OTA Live-Test +
...
### 🔴 P0 — Nächste Release (v1.17.0, MINOR — Skoda Vehicle-Info + Cross-Brand OTA Live-Test)

| Session | Version | Scope | Issues |
|---|---|---|---|
| **Skoda Vehicle-Info Bundle + Cross-Brand OTA Live-Test** | **v1.17.0** ⭐ | MINOR: (1) Skoda mysmob Endpoints aus v1.16.0 deferred: `GET /v1/vehicle-information/{vin}` (richer DeviceInfo metadata: model, modelYear, trimLevel, exteriorColour, body, engine.type, battery.capacityInKWh, maxChargingPowerInKW), `/renders` + `/equipment` für Image-Platform Erweiterung, `/v2/widgets/vehicle-status/{vin}` für lightweight battery-friendly polling Mode. (2) Cross-Brand OTA Live-Test gegen CARIAD-BFF (Audi+VW EU) + OLA (SEAT/CUPRA) software-version endpoint per `docs/RESEARCH_NOTES_2026-05-02_OTA_PROBE.md`. Wenn 200 OK: parse + sensor entities aktivieren cross-brand. (3) Charging Profile Write-Side: Number/Select Entities für target_soc_pct + max_charging_current + auto_unlock_plug pro Profile (POST/PUT endpoints). | (Live-Test asks) |
