...
|---|---|---|---|
| **Unexpected Keys Detector** | v1.8.13 | Detection-Layer der JSON-Felder loggt die unser Parser nicht liest — mit maskierten Sample-Values. Folgt `tillsteinbach/CarConnectivity-*` "Unexpected Keys found"-Pattern, das schon mehrere unserer Fixes ermöglicht hat (CC-seatcupra #109, CC-skoda #50). Scope: `cariad/_unexpected_keys.py` mit `EXPECTED_KEYS` dict, Hook im `get_status` jeder Brand, strukturierter WARN-Log + Diagnostics-Export. Privacy: alle Werte werden anonymisiert (VIN/userID/GPS-Maskierung). Ermöglicht crowd-sourced API-Discovery — User mit diversen Fuhrparks (VW Golf 7 GTE, Audi S6 2021, Audi A4 2017 etc.) können anonymisierte Reports als GitHub Issues posten | new |
