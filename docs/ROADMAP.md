...
|---|---|---|---|
| **Vehicle Data Scout** (Live API Monitor) | v1.8.13 | User-facing "Vehicle Data Scout"-Sensor der JSON-Felder loggt die unser Parser nicht liest — mit maskierten Sample-Values. Folgt `tillsteinbach/CarConnectivity-*` "Unexpected Keys found"-Pattern. Brand-übergreifender Name (DE: "API-Beobachter", FR: "Détecteur de nouvelles données" etc. — siehe strings.json). Scope: `cariad/_unexpected_keys.py` + Hook in jedem brand-client + WARN-Log + Diagnostics-Export. **Reporter Pipeline (1-Klick):** Detector findet Felder → HA Repair Notification → User klickt "Mehr Info" → Modal zeigt anonymisierte Findings → "Auf GitHub melden"-Button öffnet pre-filled Issue → User klickt Submit. ~30 Sek. KEIN Auto-Push an externe Server (GDPR + HACS-Default + GitHub ToS); opt-in Telemetry ist eigene v1.9.x Session. Privacy: alle Werte werden anonymisiert. Ermöglicht crowd-sourced API-Discovery — User mit diversen Fuhrparks (VW Golf 7 GTE, Audi S6 2021, Audi A4 2017 etc.) | new |
