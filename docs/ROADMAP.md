...
**Last updated:** 2026-05-02 — post v1.17.0 (Operational Hardening
Bundle: quota-protective poll-defaults + deactivated-vehicle
notification + year-rollover unit tests + FAQ.md + HACS-CHECKLIST.md
+ 3 new research docs + 8 upstream issue drafts + Bruno Collection
outreach). Nächste P0: v1.17.x picks aus pycupra-deep-dive (find_path,
exception taxonomy, requests_remaining sensor) + #36 Navigation aus
Bruno-Collection. Dann v1.18.0 Push Bundle.
...
### 🔴 P0 — Nächste Release (v1.17.x, PATCH — quick wins from research deep-dives)

| Session | Version | Scope | Issues |
|---|---|---|---|
| **Bruno-driven Quick Wins** | **v1.17.x** ⭐ | PATCH/MINOR aus dem Bruno-Endpoint-Catalog (post-vollscan): (1) **#36 Navigation closer** — `PUT /v1/users/vehicles/{vin}/destination` als neuer Service `vag_connect.send_destination` mit POI-Body (city, country, state, street, lat/lon, name) für SEAT/CUPRA. (2) **Auxiliary Heating** (Webasto Remote-Start) — `POST /v1/vehicles/{vin}/auxiliary-heating/{start\|stop}` mit SecToken als 2 neue Services + Switch. (3) **Battery Care endpoint** — `GET /v1/vehicles/{vin}/charging/battery-care` als read-only sensor (komplettiert v1.15.0 cap-id Arbeit). | #36 |
| **Pycupra-driven Hardening** | **v1.17.x** | PATCH aus pycupra-deep-dive (parallel oder bundle mit Bruno): port `find_path()` als `cariad/_safe_path.py`, extend exception taxonomy mit `PyCupraThrottledException`-equivalent, surface `X-RateLimit-Remaining` als `requests_remaining_today` sensor. | (HACS-Checklist outstanding) |
